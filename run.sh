...
docker build -t order-platform-msa-review .

docker stop review > /dev/null 2>&1 || true
docker rm review > /dev/null 2>&1 || true

docker run --name review \
    ...
    -d order-platform-msa-review


# Check container status
docker ps -f "name=review"